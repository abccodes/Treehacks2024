...
export const addPatient = mutation({
  // Validators for arguments.
  // PatientID: v.string(), // This is automatically generated by the database.
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert("Patient", args);
...
export const addJournalEntry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user) {
      ...
      const patientSelection = await ctx.db
        .query("Patient")
        .filter((q) => q.eq(q.field("_id"), args.PatientID))
        .first();

      console.log("PATIENT SELECT ", patientSelection);
